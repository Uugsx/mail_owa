...
const WebviewPane: React.FC<WebviewPaneProps> = ({ account, isActive }) => {
  ...
  useEffect(() => {
    ...
    const handleDOMReady = () => {
      ...
      // Direct polling from renderer: query left menu for Inbox count
      const pollCountFromRenderer = () => {
        try {
          const js = `(() => {
            function readInboxCount() {
              try {
                const elements = Array.from(document.querySelectorAll('span, div, a, p, [role="treeitem"]'));
                let best = 0;
                for (const el of elements) {
                  ...
                  // Match exactly "входящие" or "inbox"
            ...
            return readInboxCount();
          })()`;
          webview.executeJavaScript(js).then((count: any) => {
            const n = typeof count === 'number' ? count : parseInt(String(count || '0'), 10) || 0;
            if (window.parent) {
              window.parent.postMessage({ type: 'webview-message', channel: 'unread-count', data: n, accountId: account.id }, '*');
            }
          }).catch(() => {});
        } catch {}
      };
      
      // Initial quick poll and periodic
      setTimeout(pollCountFromRenderer, 3000);
      const pollTimer = setInterval(pollCountFromRenderer, 10000);
      (webview as any).__pollTimer = pollTimer;
